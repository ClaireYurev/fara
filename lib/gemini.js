// ─── Google Gemini 2.5 Pro caller ────────────────────────────────────────────
// Used as fallback when OpenAI fails. Same input contract as openai.js.
// Gemini 2.5 Pro includes an internal "thinking" pass before generating its
// response — similar to o3 — so timeout and output token budget are elevated.

const MODEL        = 'gemini-2.5-pro';
const TIMEOUT_MS   = 120000; // 2 min — thinking pass can take 30-90 s
...
export async function callGemini({ apiKey, systemPrompt, userPrompt, imageAttachments = [] }) {
  ...
  const body = {
    ...
    generationConfig: {
      temperature:     0.4,
      maxOutputTokens: 16000, // elevated to give the thinking pass room to work
    },
    safetySettings: [
